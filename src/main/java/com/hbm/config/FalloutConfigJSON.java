...
public class FalloutConfigJSON {
	...
	public static class FalloutEntry {
		...
		public boolean eval(World world, int x, int y, int z, Block b, int meta, double dist) {
			...
			if(matchesMaterial != null || matchesOpaque) {
				pos = new BlockPos(x, y, z);
				state = world.getBlockState(pos);
			}

			if(matchesMaterial != null && state.getMaterial() != matchesMaterial) return false;
			if(matchesMeta != -1 && meta != matchesMeta) return false;
			if(matchesOpaque && !state.isOpaqueCube()) return false;
			...
			if(conversion != null) {
				...
				world.setBlockState(pos, conversion.block.getStateFromMeta(conversion.meta), 3);
				return true;
			}
		...
		public void write(JsonWriter writer) throws IOException {
			if(matchesBlock != null) writer.name("matchesBlock").value(String.valueOf(matchesBlock.getRegistryName()));
		...
		private static FalloutEntry readEntry(JsonElement recipe) {
			...
			if(obj.has("matchesBlock")) entry.mB(Block.REGISTRY.getObject(new ResourceLocation(obj.get("matchesBlock").getAsString())));
		...
		private static void writeMetaArray(JsonWriter writer, Triplet<Block, Integer, Integer>[] array) throws IOException {
			...
			for(Triplet<Block, Integer, Integer> meta : array) {
				writer.beginArray();
				writer.value(String.valueOf(meta.getX().getRegistryName()));
		...
		private static Triplet<Block, Integer, Integer>[] readMetaArray(JsonElement jsonElement) {
			...
			for(int i = 0; i < metaArray.length; i++) {
				...
				metaArray[i] = new Triplet(Block.REGISTRY.getObject(new ResourceLocation(mBArray.get(0).getAsString())), mBArray.get(1).getAsInt(), mBArray.get(2).getAsInt());
			}
