...
- Скелет проекта, creative tabs (9 шт.) + кастомный фон вкладки бомб
- Регистрация блоков (1110) и айтемов (1929) — заглушки `new Item()`/`new Block()`
- JEI-интеграция (soft dependency)
- Конфиг Системы 1 + 2 + 3 (Forge `Configuration` + JSON через Gson)
- Визуал: текстуры блоков и айтемов, blockstate/model JSON, lang (10 языков)
- Кастомные классы блоков — руды: `BlockOre`, `BlockOreOutgas`, `BlockGas`; 64 руды + 11 outgas-вариантов (уран/плутоний/асбест) + 9 газовых блоков; дроп, удача, SolarSystem.Body тултип

Впереди — кастомные классы блоков (остальные: `BlockEnumMulti` и др.), TileEntity, энергосети, флюидные сети, рецепты, worldgen, рендер, оружие Sedna, космос.
